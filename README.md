...
**Fixed:** changed the position of RampInputSelect--dropdown-container to be absolute instead of fixed
...
**Fixed:** using onClick on the label instead of onChange of the hidden input
...
**Fixed:** The bug was fixed by updating the fetchAll function to concatenate the new transactions with the existing transactions
...
**Fixed:** I used the loading state from useEmployees instead of isLoading as it is related to transactions not employees.
...
**Fixed:** The view button will be visible only if the nextPage wasn't null
##### Part 2
...
**Fixed:** The view button will be visible only if transactionsByEmployee was null
